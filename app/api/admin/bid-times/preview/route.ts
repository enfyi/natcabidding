...
import { BidTimeImportError, parseBidTimeImport } from '@/lib/bid-time-import'
...
export async function POST(request: Request) {
  ...
  try {
    return Response.json(await parseBidTimeImport(file))
  } catch (error) {
    if (error instanceof BidTimeImportError) {
      return Response.json({ error: error.message, issues: error.issues }, { status: 422 })
    }
    console.error('[bid-time-import] Preview failed', error)
