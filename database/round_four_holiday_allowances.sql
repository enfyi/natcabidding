-- Round 4: return every charged Round 1-3 holiday/in-lieu day to the
-- bidder's allowance, using the selected line's hours per day.
create or replace function private.round_four_limit_for_line(line_id uuid)
returns integer language sql stable set search_path = '' as $function$
  select case private.rdo_count_for_line(line_id) when 3 then 4 else 5 end
$function$;

create or replace function private.round_four_credit_days(year_id uuid, target_bidder_id uuid)
returns integer language sql stable set search_path = '' as $function$
  select (
    ...
    where request.bid_year_id = year_id and request.bidder_id = target_bidder_id
      and request.round_number between 1 and 3
      and request.status in ('pending', 'approved')
      and day.charged and (day.is_holiday or day.is_holiday_in_lieu)
  ) + (
    select coalesce(sum(event.credit_days), 0)::integer
    from public.leave_credit_events event
    where event.bid_year_id = year_id and event.bidder_id = target_bidder_id
      and event.round_number <= 4 and event.source = 'manual_adjustment'
  )
$function$;
...
select year_row.id as bid_year_id,
       ...
       case when line.rdo_line_id is not null
         then bidder.leave_slot_allowance
...
-- Override the active bidder submission and edit routines below.
...
begin
  ...
  if batch_round <> 1 then
    round_leave_limit := case when batch_round in (2, 3) then private.round_two_three_limit_for_line(coalesce(target_rdo_line_id, submitted_rdo_line_id)) else private.round_four_limit_for_line(coalesce(target_rdo_line_id, submitted_rdo_line_id)) end;
  ...
  if batch_round = 4 then
    ...
    available_credit_days := coalesce(available_credit_days, 0);
  end if;
...
begin
  ...
  if request_row.round_number = 1 then
    ...
    round_limit := case when request_row.round_number in (2, 3) then private.round_two_three_limit_for_line(target_rdo_line_id) else private.round_four_limit_for_line(target_rdo_line_id) end;
...
begin
  ...
  for round_no in 1..5 loop
    ...
    credit_days := 0;
    if round_no >= 4 then
      credit_days := private.round_four_credit_days(year_id, target.id);
    end if;
